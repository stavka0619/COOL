...
<str_const>\\\0 { 
    BEGIN(null_in_str);
    cool_yylval.error_msg = "String contains escaped null chracter";
    return (ERROR);
}
<str_const>\0 { 
    BEGIN(null_in_str);
    cool_yylval.error_msg = "String contains null chracter";
    return (ERROR);
}
<str_const>\n { 
    BEGIN(INITIAL);
...
<str_const>. { *string_buf_ptr = *yytext; string_buf_ptr++;
  ...
  for (i=1; yytext[i] != '\"'; i++) {
    ...
    if (yytext[i]=='\\')
    {
      if (yytext[i+1]=='b')
        ...
        string_buf[cur] = yytext[i+1];
      cur++;
      i++;
    }
    else
    {
      string_buf[cur] = yytext[i];
      cur++;
    }
  }
  string_buf[cur] = '\0';
  strcpy(yytext, string_buf);
  cool_yylval.symbol= inttable.add_string(yytext);
                    return (STR_CONST);
}
