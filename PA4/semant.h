...
class TableNode {
 ...
 TableNode(Class_ c): cls(c),  
    children(std::vector<TableNode>() ){  }
...
class ClassTable {
 ...
 public:
  ...
  void add_class(Class_ c, Classes classes) {
    if (c->get_parent()->equal_string("Bool", 4) ||
	c->get_parent()->equal_string("SELF_TYPE", 9) ||
	c->get_parent()->equal_string("String", 6) ) {
      semant_error(c);
      err() <<"Class cannot inherits Bool, SELF_TYPE, or String"<<endl;
      cerr << "Compilation halted due to static semantic errors." << endl;
      exit(1);

    }
    TableNode* parent = root.find_child(c->get_parent());
    if (parent==NULL && classes) { //check forward declaration
    ...
    if (existing) {// already added as forward declaration
      return;
    }
  ...
  // check duplicate argument name, this is very INEFFICIENT!
  ...
  void add_class_method_args(Class_ c, Symbol fun, Formals args) {
    // Firstly check whether there is arg type confliction in parent classes
    if (!is_root(c->get_name())) {
      if (!check_arg_type(root.find_child(c->get_parent())->get_class(), fun, args)) {
	semant_error(c);
	err() <<"Incompatible number or type of formal parameters in redefined function"<<endl;
	return;
      }
	...
	return;
      ...
      if (fml->get_type()->equal_string("SELF_TYPE", 9)) {
	semant_error(c);
	err() <<"SELF_TYPE cannot be the type of a formal parameter"<<endl;
	return;
      }
  ...
  Symbol find_fun_arg_type(Class_ c, Symbol fun, int i) {
    ...
    if (ret)
      return ret;
    // if not root(object) search for parent
    if (!is_root(c->get_name())) {
  ...
  Symbol find_ret_type(Class_ c, Symbol fun) {
    // find the function RECUSIVELY to its parent
    TableNode* node = root.find_child(c->get_name());
    if (node==NULL)
      return NULL;
  ...
  /* void dump_to_symtable(SymTable *table) { */
  /*   root.add_to_symtable(table); */
  /* } */
